                         ...
                         )
{
    ...
    for (auto i = 0U; i <= segment_count; ++i) {
        float next_x{std::cos(angle + angle_delta) * radius1};
        float next_y{std::sin(angle + angle_delta) * radius1};
        float next_u{0.5f + std::cos(angle + angle_delta) * 0.5f * radius1norm};
    ...
    angle = 0.0f;
    for (auto i = 0U; i <= segment_count; ++i) {
        float next_x{std::cos(angle + angle_delta) * radius2};
        float next_y{std::sin(angle + angle_delta) * radius2};
