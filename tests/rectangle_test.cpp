                         ...
                         )
{
    ...
    float segment_height{height / static_cast<float>(height_segments)};

    float half_width{width * 0.5f};
    float segment_width{width / static_cast<float>(width_segments)};

    for (auto i = 0U; i <= height_segments; ++i) {
        float y{static_cast<float>(i) * segment_height - half_height};
        for (auto j = 0U; j <= width_segments; ++j) {
    ...
    if (geometry_type == asr::GeometryType::Lines || geometry_type == asr::GeometryType::Triangles) {
        for (auto i = 0U; i < height_segments; ++i) {
            for (auto j = 0U; j < width_segments; ++j) {
                unsigned int index_a{i * (width_segments + 1) + j};
                unsigned int index_b{index_a + 1};
                unsigned int index_c{index_a + (width_segments + 1)};
