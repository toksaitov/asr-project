...
static const std::string Vertex_Shader_Source{R"( // NOLINT(cert-err58-cpp)
    ...
    void main()
    {
        fragment_color = color * instance_color;
        if (texture_enabled) {
        ...
        gl_Position = model_view_projection_matrix * instance_transform * position;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv)
{
    ...
    create_window(500U, 500U, "Instancing Test on ASR Version 1.3");
    ...
    for (unsigned int i = 0; i < ROWS; ++i) {
        for (unsigned int j = 0; j < COLS; ++j) {
            for (unsigned int k = 0; k < DEPTH; ++k) {
                ...
                glm::mat4 translation = glm::translate(glm::mat4{1.0f}, glm::vec3{x * scale, y * scale, z * scale});
    ...
    auto geometry = create_geometry(Triangles, geometry_vertices, geometry_indices, instances);
